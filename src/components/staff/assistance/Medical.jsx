...
export default function Medical({ user, accessToken }) {
  ...
  const handleConfirm = async () => {
    ...
    await axios
      .delete(`/medical?id=${deleteSelected}`, {
        headers: {
  ...
  const getData = async () => {
    await axios
      .get(`/medical/all?filter=${filter}&dateFrom=${filterDate.from}&dateTo=${filterDate.to}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
        params: {
  ...
  return (
    <div className='flex flex-col gap-2'>
      ...
      {
        !isModalOpen ? "" :
          <AssistanceForm accessToken={accessToken} user={useEffect} type={'medical'} selected={selected} setSelected={setSelected} setModal={setModal} getData={getData} />
      }
      {/* table */}
      <div className='flex flex-col gap-2 bg-white p-5 rounded-lg shadow-md'>
        <div className='flex justify-between pe-10'>
          <p className='font-bold text-lg text-slate-500'>Medical Requested List</p>
          <button
                  ...
                  schoolarship.map(data => (
                    <tr className='hover:bg-emerald-100 text-center'>
                      ...
                      <td onClick={(e) => handleDownload(data.medical_abstract, data.medical_extension)}>
                        <p className={`${data.medical_abstract ? 'hover:underline cursor-pointer text-blue-600' : ''}`}>
                          {data.medical_abstract ? "Download" : "Not Submitted"}
                        </p>
                      </td>
